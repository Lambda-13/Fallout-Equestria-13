...
/obj/structure/destructible/clockwork/massive/celestial_gateway/New()
	..()
	addtimer(CALLBACK(src, PROC_REF(spawn_animation)), 0)
		...
		if(GATEWAY_RATVAR_ARRIVAL to INFINITY)
			if(!purpose_fulfilled)
				...
				if(ratvar_portal)
					...
					addtimer(CALLBACK(SSshuttle.emergency, TYPE_PROC_REF(/obj/docking_port/mobile/emergency, request), null, 0), 0) //call the shuttle immediately
					sleep(3)
