			...
			if(iscarbon(L))
				...
				switch(firer.a_intent)
					...
					if(INTENT_GRAB)
						...
						C.throw_at(get_step_towards(H,C), 8, 2, callback=CALLBACK(src, PROC_REF(tentacle_grab), H, C))
						return 1

					if(INTENT_HARM)
						C.visible_message("<span class='danger'>[L] is thrown towards [H] by a tentacle!</span>","<span class='userdanger'>A tentacle grabs you and throws you towards [H]!</span>")
						C.Weaken(3)
						C.throw_at(get_step_towards(H,C), 8, 2, callback=CALLBACK(src, PROC_REF(tentacle_stab), H, C))
						return 1
			else
