...
/mob/living/simple_animal/hostile/megafauna/bubblegum/OpenFire()
	...
	var/warped = FALSE
	if(!try_bloodattack())
		addtimer(CALLBACK(src, PROC_REF(blood_spray)), 0)
		warped = blood_warp()
	...
	if(prob(90 - anger_modifier) || slaughterlings())
		if(health > maxHealth * 0.5)
			addtimer(CALLBACK(src, PROC_REF(charge)), 0)
		else
			if(prob(70) || warped)
				addtimer(CALLBACK(src, PROC_REF(triple_charge)), 0)
			else
				addtimer(CALLBACK(src, PROC_REF(warp_charge)), 0)
...
/mob/living/simple_animal/hostile/megafauna/bubblegum/proc/try_bloodattack()
	...
	if(targets.len)
		addtimer(CALLBACK(src, PROC_REF(bloodattack), targets, prob(50)), 0)
