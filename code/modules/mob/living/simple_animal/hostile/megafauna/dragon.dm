...
/obj/effect/overlay/temp/target/New(loc)
	..()
	addtimer(CALLBACK(src, PROC_REF(fall)), 0)
...
/mob/living/simple_animal/hostile/megafauna/dragon/OpenFire()
	...
	if(prob(15 + anger_modifier) && !client)
		if(health < maxHealth/2)
			addtimer(CALLBACK(src, PROC_REF(swoop_attack), 1), 0)
		else
			fire_rain()

	else if(prob(10+anger_modifier) && !client)
		if(health > maxHealth/2)
			addtimer(CALLBACK(src, PROC_REF(swoop_attack)), 0)
		else
			addtimer(CALLBACK(src, PROC_REF(triple_swoop)), 0)
	else
		fire_walls()
...
/mob/living/simple_animal/hostile/megafauna/dragon/proc/fire_walls()
	...
	for(var/d in cardinal)
		addtimer(CALLBACK(src, PROC_REF(fire_wall), d), 0)
