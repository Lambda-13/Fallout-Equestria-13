...
/obj/item/clothing/head/helmet/clockwork/equipped(mob/living/user, slot)
	...
	if(slot == slot_head && !is_servant_of_ratvar(user))
		...
		addtimer(CALLBACK(user, TYPE_PROC_REF(/mob/living, unEquip)), src, 1) //equipped happens before putting stuff on(but not before picking items up), 1). thus, we need to wait for it to be on before forcing it off.
...
/obj/item/clothing/suit/armor/clockwork/equipped(mob/living/user, slot)
	...
	if(slot == slot_wear_suit && !is_servant_of_ratvar(user))
		if(!iscultist(user))
			...
			user.IgniteMob()
		addtimer(CALLBACK(user, TYPE_PROC_REF(/mob/living, unEquip), src, 1), 1)
