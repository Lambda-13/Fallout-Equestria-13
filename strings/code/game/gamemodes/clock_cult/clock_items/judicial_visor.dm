...
/obj/effect/proc_holder/judicial_visor/InterceptClickOn(mob/living/caller, params, atom/target)
	...
	if(target in view(7, get_turf(ranged_ability_user)))
		...
		for(var/obj/item/clothing/glasses/judicial_visor/V in ranged_ability_user.GetAllContents())
			...
			V.update_status()
			addtimer(CALLBACK(V, TYPE_PROC_REF(/obj/item/clothing/glasses/judicial_visor, recharge_visor)), (ratvar_awakens ? visor.recharge_cooldown*0.1 : visor.recharge_cooldown) * 2, ranged_ability_user)
		...
		addtimer(CALLBACK(visor, TYPE_PROC_REF(/obj/item/clothing/glasses/judicial_visor, recharge_visor)), ratvar_awakens ? visor.recharge_cooldown*0.1 : visor.recharge_cooldown, ranged_ability_user)//Cooldown is reduced by 10x if Ratvar is up
		remove_ranged_ability()
...
/obj/effect/clockwork/judicial_marker/New(loc, caster)
	...
	set_light(4, 3)
	user = caster
	addtimer(CALLBACK(src, PROC_REF(judicialblast)), 0)
